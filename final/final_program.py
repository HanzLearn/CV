...
def log_summary(output_dir, total_frames, total_objects, total_racks, csv_file, avg_fps):
    """
...
def generate_planogram_df_from_racks(df_rack1, df_rack2, df_rack3):
    ...
    - Extracting the first row per valid track
    - Inserting blank rows between rack_id changes and below the header

    Args:
        df_rack1, df_rack2, df_rack3 (pd.DataFrame): DataFrames containing detection results.
            Each must include: ["frame_id", "rack_id", "track_id", "class_id", "class_name", "cx"]

    Returns:
        pd.DataFrame: Formatted planogram DataFrame.
    """
    # Concatenate all rack DataFrames
    df_all = pd.concat([df_rack1, df_rack2, df_rack3], ignore_index=True)

    # Filter valid entries (track_id must appear ≥30 times)
    valid_entries = []
    ...
    # Insert blank rows between header and first item, and between rack_id changes
    rows_with_blanks = []
...
# Comparison to expected planogram
